package info.mancy.android.wordpress.xmlrpc;
...
class XMLRPCSerializer {
	...
	static void serialize(XmlSerializer serializer, Object object)
		...
		if (object instanceof Integer || object instanceof Short
			...
			serializer.startTag(null, TYPE_DATE_TIME_ISO8601).text(sDate)
			...
			InputStream inStream = new DataInputStream(new FileInputStream(
			...
			while ((length = inStream.read(buffer)) > 0) {
				chunk = Base64
						.encodeToString(buffer, 0, length, Base64.DEFAULT);
